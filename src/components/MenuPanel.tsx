        ...
        return (
          <div className="grid max-h-[46vh] grid-cols-3 gap-2.5 overflow-y-auto pr-1 sm:grid-cols-6">
            {(Object.keys(HORDE_NAME) as (keyof typeof HORDE_NAME)[]).map((k) => (
              <div key={k} className="pop-card rounded-2xl p-2 text-center">
                <Sprite src={HORDE_SRC[k][0]} className="mx-auto h-14 w-10" />
                <p className="pop-label mt-1 truncate text-[10px] font-black uppercase">
                  {HORDE_NAME[k]}
                </p>
                <p className="text-[9px] font-black uppercase text-gold">Tier {HORDE_TIER[k]}</p>
              </div>
            ))}
          </div>
        );
