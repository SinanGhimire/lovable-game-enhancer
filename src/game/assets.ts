...
export type ActorKey = BaseActorKey | HordeKey | CritterEnemyKey;
...
const STATIC_ACTOR_SRC: Record<string, [string, string, string]> = {
  ...
  // Gameplay variants reuse the four thick-outlined creatures visible on the
  // home screen so every wave belongs to the same visual world.
...
const ACTOR_KEYS: ActorKey[] = [
  ...(Object.keys(STATIC_ACTOR_SRC) as ActorKey[]),
  ...(CRITTER_ENEMIES.map((d) => d.key) as ActorKey[]),
];

function actorSrc(key: ActorKey): [string, string, string] {
  const stat = STATIC_ACTOR_SRC[key];
  if (stat) return stat;
...
export function loadSprites(): Promise<Sprites> {
  ...
  inflight = (async () => {
    ...
    const [actorAnims, playerAnims, singleImgs] = await Promise.all([
      Promise.all(actorKeys.map((k) => loadAnims(actorSrc(k)))),
